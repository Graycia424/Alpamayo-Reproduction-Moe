...
"""
本文件属于 Alpamayo 项目。
主要功能：实现有关视觉输入（视频/图片）特征预处理和相机配置的辅助工具脚本 (`helper.py`)。
数据流与模块关系说明：
1. 【输入来源】：接受由 `load_physical_aiavdataset.py` 处理过或者本地保存的原始相机矩阵以及系统默认参数（分辨率等）。
2. 【核心处理】：针对基础骨干网（如 Qwen-VL 等）对图像大小、分辨率的特别要求，通过字典和处理器对视觉模态进行特征标准化、Prompt组装对齐等辅助变换。
3. 【输出去向】：变换或格式化完成的 Tokens 与图像/视频特征最后供 `inference.py` 或者 `models/alpamayo1_5.py` 的推理过程使用。
"""
...
MIN_PIXELS = 163840
MAX_PIXELS = 196608
BASE_PROCESSOR_NAME = "/path/to/data/Qwen3-VL-2B-Instruct"
