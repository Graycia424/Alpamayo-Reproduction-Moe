...
"""
本文件属于 Alpamayo 项目。
主要功能：实现结果的 BEV (鸟瞰图) 轨迹可视化工具 (`viz_utils.py`)，绘制包含多路摄像机输入与最终预测分布的效果图。
数据流与模块关系说明：
1. 【输入来源】：接受由 `inference.py` 输出的模型预测 3D/2D 坐标（预测的自车历史或未来推断轨迹点）和原始的多视角图片阵列。
2. 【核心处理】：利用 `matplotlib` 和 `seaborn` 将环境视觉场景张量渲染为图像格子（Camera Grid），同时将实数格式的轨迹位姿点云用颜色和线条覆写到俯视图板上。
3. 【输出去向】：最终绘制并保存出结果图像（Image/Figure 对象）于执行环境中供保存或者直接进行结果 Review 等目的，本模块不输出数据返回给任何核心模型网络。
"""
