...
"""
本文件属于 Alpamayo 项目。
主要功能：构建并封装用于 VLA 推理任务的多模态数据集批处理（batch）接口。
数据流与模块关系说明：
1. 【输入来源】：通过 `dataset.py` 获取底层的原始物理传感器数据集切片；同时调用相关的 `video.py`, `egomotion.py`, 以及 `interpolation.py`。
2. 【核心处理】：将离散或包含物理特性的车辆位姿信息(`PhysicalAIAVDatasetInterface`)，对齐(interpolate)、解析并包装成深度学习能够处理的一个 batch 张量映射字典。
3. 【输出去向】：最终产生的时间序列位姿、摄像头输入张量、及对应的 prompt 等，会被直接传送给下游的 `inference.py`（或执行环境中的推理任务）并最终喂给 Transformer (`models/alpamayo1_5.py`) 架构。
"""
