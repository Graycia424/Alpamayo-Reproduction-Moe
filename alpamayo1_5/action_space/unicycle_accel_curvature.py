...
"""
本文件属于 Alpamayo 项目。
主要功能：具体基于“独轮车动力学模型”实现车辆状态在加速度(Accel)和曲率(Curvature)层面的动作转化计算逻辑 (`UnicycleAccelCurvature`)。
数据流与模块关系说明：
1. 【输入来源】：接受由 `egomotion.py` 以及场景采集来的三维车辆历史轨迹点的 x, y 与旋转（yaw）坐标输入以及时间标量。
2. 【核心处理】：继承 `ActionSpace` (`action_space.py`) 并利用 `utils.py` 里底层的微积分求导，计算自车在前方的纵向加速度(a)、转向曲率(k)。从而使得网络推演目标不再是一个不可微的点，而是车辆运动能力上限范围内的控制参数。
3. 【输出去向】：产生的曲率、加速度张量数据会输入给上述的动作空间分词器(`discrete_action_space.py`)量化，或者在推理后从加速度/曲率累加推导重构成三维空间未来轨迹，输送给 `viz_utils.py` 或者下游规划控制算法。
"""
