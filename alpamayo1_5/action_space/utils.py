...
"""
本文件属于 Alpamayo 项目。
主要功能：提供针对物理运动动作空间转换所需的底层数学工具函数（Utils）。
数据流与模块关系说明：
1. 【输入来源】：接受由 `unicycle_accel_curvature.py` 传入的张量或角度偏移量和位置数组。
2. 【核心处理】：以纯数学运算（去包裹角度 `unwrap_angle`、矩阵方程求解 `solve_xs_eq_y` 以及位移偏航角微积分推演速度 `dxy_theta_to_v`）保证三维空间和运动学方程的严谨正确性。
3. 【输出去向】：运算结果原地返回，被上述的动作解析主模型直接应用，不与其他大网络架构直接进行耦合。
"""
