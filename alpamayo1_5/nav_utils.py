...
"""
本文件属于 Alpamayo 项目。
主要功能：处理与“自然语言导航指令”及轨迹预测干预对比分析相关的文本工具 (`nav_utils.py`)。
数据流与模块关系说明：
1. 【输入来源】：来自主调用程序传递的人类自然语言 Prompt 或者自动生成的路口/转向等描述指令。
2. 【核心处理】：进行导航指令语义级干预，如字符串匹配和替换来实施方向翻转（将其倒转为反事实的反方向指令 `swap_direction`），并以此管理三个预设推演条件的联合比较批处理（带导航/无导航/反事实导航）。
3. 【输出去向】：最终组装替换过的 Prompt 以及包装过的 Batch 数据传入大语言模型 `inference.py` 或者 `models/alpamayo1_5.py` 的推论循环，用来比较或评估模型是否能深度理解方向和导航 Prompt 并作出对应的正确行车轨迹相应。
"""
