...
"""
本文件属于 Alpamayo 项目。
主要功能：定义 Diffusion（扩散模型）相关基础抽象模式与通用去噪接口协议。
数据流与模块关系说明：
1. 【输入来源】：接受模型配置初始化参数（无数据态），为 `diffusion/` 包的其他子模块提供标准框架。
2. 【核心处理】：定义了 `StepFn` 以及时间步调度采样等高度抽象的去噪基类功能（可能用于 Diffusion Action Policy 中的噪音与状态过渡流形约束）。
3. 【输出去向】：作为基类，被同目录下的 `flow_matching.py` 等更特定的实现继承并具体化计算逻辑。
"""
