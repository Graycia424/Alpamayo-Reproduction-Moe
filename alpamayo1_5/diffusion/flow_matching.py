...
"""
本文件属于 Alpamayo 项目。
主要功能：实现扩散模型中的流匹配思想（Flow Matching）类。
数据流与模块关系说明：
1. 【输入来源】：继承自 `base.py` 中 `BaseDiffusion`，接收带有高斯噪声或源分布（通常代表先验分布/随机噪声）的张量表示流以及模型前向的 `StepFn`（生成预测导数/速度向量的网络部分）。
2. 【核心处理】：基于常微分方程 (ODE) 实现目标分布到概率路径上的线性插值运算和逐步计算 (Euler method)，从而生成目标分布 (如合理的连续 Action 空间动作或图像特征)。
3. 【输出去向】：生成的去噪后张量能够反映目标特征，并传输回模型采样流程、或者输出给上层的 VLA 动作网络实现连续 Action/Policy 输出。
"""
