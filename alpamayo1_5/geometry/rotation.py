...
"""
本文件属于 Alpamayo 项目。
主要功能：处理车辆欧拉角、旋转矩阵、四元数等三维几何属性与位姿的计算及变换 (`rotation.py`)。
数据流与模块关系说明：
1. 【输入来源】：通常接收来自 `egomotion.py` 或者 `action_space/` 处理好的 3×3 旋转矩阵或者 2D 平面旋转角度。
2. 【核心处理】：无状态（Stateless）地在各类数学表示（如欧拉角的 Yaw 偏航角、两点之间弧度差、SO(3) 特殊正交群矩阵等）之间进行严密的转换。处理中依赖了 `scipy.spatial.transform.Rotation` 工具计算。
3. 【输出去向】：原地输出并返回转换后的四元数或者角度张量，由调用它的高层控制器逻辑、或者是绘制图表的视觉层接收。
"""
