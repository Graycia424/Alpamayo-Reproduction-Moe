...
"""
本文件属于 Alpamayo 项目。
主要功能：负责从压缩的 mp4 视频流里精准抽取特定时间戳的图片帧，可以理解为一个“按帧截图器”。

【与其他文件的依赖调用关系】
- 向上依赖（本文件调用了谁）：
  没有调用项目里的其他文件，主要依赖第三方的 `av` (PyAV) 库来做视频解码。
- 向下提供（谁调用了本文件）：
  1. 被 `dataset.py` 调用：当读取数据集时，只要涉及到“需要哪几秒的相机画面”，`dataset.py` 就会调用这里的功能把视频字节流变成 Numpy 的图像数组。
"""
import abc
import collections
import io
