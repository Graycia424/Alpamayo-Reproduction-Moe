...
主要功能：评估由 `train_decoder_expert.py` 训练出的聚合大专家（Decoder Expert）。
该脚本会利用提取出的 KV Cache (prefill cache) 进行轨迹采样，并计算 minADE 误差。

【和其他文件的依赖调用关系】
向上依赖（调了谁）：
- 依赖 `train_decoder_expert.py`：使用其中的 `AlpamayoTrainDataset` 数据集构建逻辑与 `build_model_with_light_expert`。
- 依赖 `alpamayo_r1.helper`：用于处理多模态数据的输入组装。
"""
...
def _expand_cache(prefill_cache, num_traj_samples: int):
    """Expand KV cache batch dim by num_traj_samples."""
    from transformers import DynamicCache
    expanded = DynamicCache()
    # DynamicCache stores layers as list of (key, value) tuples accessible via __iter__
...
def sample_trajectories_from_prefill_cache(
    ...
    """Sample trajectories using prefill KV cache (before CoC generation)."""
    B = hist_xyz.shape[0]
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, default="./decoder_expert_checkpoints/checkpoint-3000/decoder_expert.pt",
                        help="Path to decoder_expert.pt")
    ...
    parser.add_argument("--output", type=str, default="eval_decoder_expert_results.csv")
