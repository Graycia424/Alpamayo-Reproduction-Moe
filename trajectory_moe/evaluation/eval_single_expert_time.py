# -*- coding: utf-8 -*-
"""
本文件属于 Alpamayo r1项目的轨迹 MoE 模块。
主要功能：评估单个轻量级专家在使用多组时序偏移（t0_offsets_us，横跨 2秒到12秒）预测场景下的综合稳定性，间接输出耗时和指标评估。

【和其他文件的依赖调用关系】
向上依赖（调了谁）：
- 依赖 `trian_single_expert.py` 与局部的数据读取函数。
"""
...
def evaluate(args):
    ...
    dataset = AlpamayoTrainDataset(clip_ids=clip_ids, t0_offsets_us=[2_000_000, 4_000_000, 6_000_000, 8_000_000, 10_000_000, 12_000_000])
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, default="./light_expert_checkpoints_chunk200/checkpoint-10000/light_expert.pt")
