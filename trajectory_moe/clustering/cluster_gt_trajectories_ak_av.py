...
- 浣跨敤瀹樻柟 UnicycleAccelCurvatureActionSpace 鎻愬彇鍔犻€熷害鍜屾洸鐜?
- 鍩轰簬鏇茬巼鍧囧€肩殑鍒嗕綅鏁板垎绠辫仛绫伙紝纭繚姣忎釜绨囧ぇ灏忓钩鍧?
- 鍙鍖栦粛浣跨敤xy杞ㄨ抗鍧愭爣
"""
...
def cluster_trajectories(actions, n_clusters=4):
    """基于加速度和曲率均值的 2D 分位数网格分箱，确保每个簇大小平均。"""
    accel = actions[:, :, 0]      # (N, 64) 鍔犻€熷害
    curvature = actions[:, :, 1]  # (N, 64) 鏇茬巼
    
    mean_accel = np.mean(accel, axis=1)        # (N,)
    mean_curvature = np.mean(curvature, axis=1)  # (N,)
    
    # 2D鍒嗕綅鏁扮綉鏍? 鍔犻€熷害2妗?x 鏇茬巼2妗?= 4绨?
    accel_bin = (mean_accel >= np.median(mean_accel)).astype(int)  # 0鎴?
    curv_bin = (mean_curvature >= np.median(mean_curvature)).astype(int)  # 0鎴?
    labels = accel_bin * 2 + curv_bin  # 0,1,2,3
...
def visualize_pca_distribution(flat_norm, labels, output_dir):
    ...
    plt.xlabel("PCA-1")
    plt.ylabel("PCA-2")
    plt.title("Trajectory clusters in PCA space (curvature quantile)")
    plt.grid(True)
    plt.legend()
...
def main():
    ...
    parser.add_argument("--output-dir", default="gt_clustering_results_ak_av_3")
    ...
    t0_offsets_us = list(range(2_000_000, 13_000_000, 1_000_000))
    ...
    logger.info(f"寮€濮?D鍒嗕綅鏁扮綉鏍艰仛绫?(n_clusters=4, 鐗瑰緛=鍔犻€熷害+鏇茬巼鍧囧€?...")
