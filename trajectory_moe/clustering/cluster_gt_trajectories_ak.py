...
def cluster_trajectories(actions, n_clusters=3):
    ...
    # 浜岀淮鐗瑰緛缁勫悎
    features = np.stack([mean_accel, mean_curvature], axis=1)  # (N, 2)
    ...
    # 杩斿洖瀹屾暣actions鐨勬爣鍑嗗寲鐗堟湰鐢ㄤ簬PCA鍙鍖?
    flat = actions.reshape(len(actions), -1)  # (N, 128)
    flat_scaler = StandardScaler()
    flat_norm = flat_scaler.fit_transform(flat)
    
    return labels, kmeans, scaler, flat_norm
...
def visualize_pca_distribution(flat_norm, labels, output_dir):
    ...
    plt.xlabel("PCA-1")
    plt.ylabel("PCA-2")
    plt.title("Trajectory clusters in PCA space (accel+curvature)")
    plt.grid(True)
    plt.legend()
...
def main():
    ...
    parser.add_argument("--output-dir", default="gt_clustering_results_ak_3")
    ...
    logger.info(f"寮€濮婯Means鑱氱被 (n_clusters={args.n_clusters}, 鐗瑰緛=鍔犻€熷害+鏇茬巼鍧囧€?...")
