...
主要功能：基于 Alpamayo-R1 架构的 Macro-MoE 第三阶段核心任务：联合微调 (Joint Fine-tuning，单卡训练版本)。
将冻结的 VLM 与新训练的 Router 和若干 Experts 组合，利用 Gumbel-Softmax 实现可微分的端到端专家选择，联合优化路由损失(L_routing / 交叉熵)与轨迹损失(L_trajectory / Flow-matching MSE)。

【和其他文件的依赖调用关系】
向上依赖（调了谁）：
- 依赖 `train_decoder_expert.py` 获取轻量专家模型定义与配置。
- 依赖 `train_router.py` 获取路由器结构。
"""
...
def train(args):
    ...
    expert_paths = [
        f"{args.expert_dir}/expert_{i}/checkpoint-9000/decoder_expert.pt" for i in range(args.num_experts)
    ]
    ...
    logger.info("MoE trainable parameters: %d (router + %d experts)", trainable, args.num_experts)
...
def _save_moe(router, experts, in_projs, out_projs, path: Path):
    ...
    torch.save(router.state_dict(), path / "router.pt")
    for i in range(len(experts)):
        torch.save({
            "expert": experts[i].state_dict(),
            "action_in_proj": in_projs[i].state_dict(),
            "action_out_proj": out_projs[i].state_dict(),
        }, path / f"expert_{i}.pt")
