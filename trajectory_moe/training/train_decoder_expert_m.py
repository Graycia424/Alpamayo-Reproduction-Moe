...
主要功能：基于 Alpamayo-R1 架构进行轻量级专家（Decoder Expert）的训练（多卡 DDP 并行训练版本）。
核心流程：提取模型前缀阶段（即生成 CoC 思维链之前，VLM prefill 阶段）的 KV Cache 作为视觉与历史特征表示，采用 Flow-Matching（流匹配）技术训练出一个专门的轻量接力网络。

【和其他文件的依赖调用关系】
向上依赖（调了谁）：
- 依赖最新的 `alpamayo_r1.models.alpamayo_r1` 基座（彻底弃用 1.5 版本）。
- 本地 `dataset` 加载通用物理数据集接口。
"""
        ...
        for batch in dataloader:
            ...
            with torch.autocast(device_type=device_type, dtype=dtype):
                ...
                loss = _compute_fm_loss(
                    raw_model, gt_actions, prefill_cache, n_diffusion_tokens, device, dtype
                )
            ...
            if local_rank == 0:
                ...
                if step % args.save_every == 0:
                    ...
                    _save_decoder_expert(raw_model, save_path)
                    logger.info("Saved checkpoint to %s", save_path)

    if local_rank == 0:
        final_path = Path(args.output_dir) / "final"
        _save_decoder_expert(raw_model, final_path)
...
def _compute_fm_loss(
    ...
    forward_kwargs = {}
    if model.config.expert_non_causal_attention:  # type: ignore[union-attr]
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./decoder_expert_checkpoints_m")
