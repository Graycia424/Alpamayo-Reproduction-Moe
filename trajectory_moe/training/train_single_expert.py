...
主要功能：基于 Alpamayo-R1 架构训练一个轻量级替换专家节点（单卡训练版本）。
【关键差异】：它在提取 VLM KV Cache 时，是包含完整的思维链文本（After CoC generation）的，而 `train_decoder_expert.py` 则是在文本生成前（prefill）截断提取的。

【和其他文件的依赖调用关系】
向上依赖（调了谁）：
- 依赖 `alpamayo_r1.models.alpamayo_r1` 与对应特殊的 Logits 配置工具（以处理自回归流的断点控制）。
"""
        ...
        for batch in dataloader:
            ...
            with torch.autocast(device_type=device, dtype=dtype):
                ...
                loss = _compute_fm_loss_with_coc(
                    model, gt_actions, prompt_cache, vlm_outputs.sequences,
            ...
            if step % args.save_every == 0:
                ...
                _save_light_expert(model, save_path)
                logger.info("Saved checkpoint to %s", save_path)

    final_path = Path(args.output_dir) / "final"
    _save_light_expert(model, final_path)
...
def main():
    ...
    parser.add_argument("--chunk-end", type=int, default=100)
